...
Comme pour l'algorithme MCMC, l'algorithme SMC (Sequential Monte-Carlo) - ou filtre particulaire - a pour objectif de simuler une trajectoire de $\xi_x$ à partir des observations $Y$ et des paramètres $\theta$.
...
Pour la version MCMC nous avons réalisé $Q = 500$ itérations de l'algorithme SAEM et après plusieurs tests et analyses graphiques, $\alpha_{min}$ a été fixé à $90$ et $M_{max}$ à $20$, ce qui correspond à des ordres de grandeur régulièrement employés dans la littérature.
...
Comme nous pouvons le voir sur les Figures \@ref(fig:coef1-SAEM-smc) et \@ref(fig:coef2-SAEM-smc), l'utilisation d'une étape SMC accélère sensiblement la convergence de $\hat{\theta}$, aussi nous avons utilisé seulement $Q = 100$ itérations de l'algorithme SAEM et nous avons diminué $\alpha_{min}$ à 25.
L'étape SMC implique $P = 500$ particules.

La Figure \@ref(fig:result-SAEM-smc) montre que cette version de SAEM permet une reconstruction de $\xi_x$ légèrement meilleure que la version MCMC, comme lorsque nous estimions $\hat{\xi_x}$ en fixant les paramètres $\theta$.
...
En revanche la Figure \@ref(fig:coef1-SAEM-smc) suggère que les estimations $\hat{\psi}$ et $\hat{\gamma}$ sont moins bonnes que précédemment : la convergence se fait vers des valeurs respectivement inférieure et supérieure à celles attendues.
...
Enfin, la Figure \@ref(fig:coef2-SAEM-smc) indique que les paramètres $\hat{A}, \hat{B}, \hat{b}$ convergent mieux vers les valeurs fixées des paramètres avec la version SMC de l'algorithme.
...
Cependant, nous remarquons que les estimations des paramètres via la méthode SMC varient plus et sont parfois très éloignées de la valeur réelle.
...
Cependant, nous avons noté que l'implémentation en **R** de l'algorithme avec l'étape SMC est bien plus rapide que celle avec l'étape MCMC : les $1000$ répétitions du plan d'expérience SMC ont été exécutées en un peu moins de 15 minutes, contre plus d'une heure pour le MCMC.
...
\qquad Les narvals mâles possèdent une grande dent avec des capacités sensorielles très développées. A travers cette dent, nous avons chercher à estimer la durée de vie des narvals. En effet, leur dent comporte des marqueurs saisonniers au cours de leur croissance créant des motifs saisonniers variant en fonction des saisons. Notre objectif etait donc d'estimer au mieux cette sinusoïde à l'aide d'un modèle SAEM. 
Le modèle SAEM est basé sur la log-vraisemblance en effectuant k itération en actualisant la valeur au temps t en fonction de celle en t-1. Nous avons effectuer l'algorithme SAEM en implémentant deux algorithmes distincts, MCMC et SMC afin d'estimer une trajectoire $\xi$ à partir des observations $Y$ ainsi que des paramètres $\theta$: 
\item L'algorithme MCMC (Markov Chain Monte Carlo) est un algorithme de Gibbs - Metropolis Hasting avec marche aléatoire consiste à accepter ou de rejeter un candidat pour chaque instant du processus selon un rapport de vraisemblance..
\item L'algorithme SMC (Sequential Monte Carlo ) propose pour chaque instant $P$ réalisation en fonction des $P$ estimations réalisées à l'instant précédent. A partir de ces $P$ particules est associé un poids égale à la probabilité d'observer la valeur de $Y$ à l'instant courant associe conditionnellement à la réalisation simulée.

\qquad Nous avons donc obtenues deux estimations des paramètres différents via l'algorithme SAEM, menant à des modèlisation différentes. En vu de nos paramètres fixée, nous avons pus comparer nos éstimations obtenues avec la réalite.

\qquad L'estimation des paramètre via les deux méthodes sont différentes. En effet, l'estimation des paramètres pour les deux méthodes semblent être assez bonnes de manières générale, hormis $\hat\gamma$ qui est sous estimé. Nous remarquons que la méthode MCMC permet mieux d'estimer $\hat\psi$, $\hat\gamma$ ainsi que $\hat{a}$, tandis que la l'algorithme SMC permet de mieux estimer $\hat\omega$, $\hat{A}$, $\hat{B}$ et $\hat{b}$. Cependant, via le plan d'éxperience,  nous pouvons voir que l'estimations des paramètres via la mèthode SMC est bien moins stable, et estime parfois des valeurs de parametre abhrérentes nottament pour $\hat\omega$, $\hat\gamma$ ou encore $\hat{b}$. Un point non négligeable favorisant l'algorithme SMC est les temps d'execution : Il est environ cinq fois plus rapide que l'algorithme MCMC.

\qquad Pour conclure, l'estimation des paramètres via les deux algorithme est bonne et la modèlisation est présice. Chacun des deux algorithme nous apporte des avantages et inconveniant différents. Cependant, malgrès des optimisations à effectuer,l'algorithme SMC semble être l'algorithme le plus adapté pour notre modélisation SAEM, pour son éstimation des paramètres et son gain de temps. 
