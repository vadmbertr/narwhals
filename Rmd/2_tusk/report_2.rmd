...
- étape E : calcul de $Q(\theta, \theta^k)$, l'espérance de la log-vraisemblance du modèle,
...
Nous pouvons observer sur la Figure \@ref(fig:sim-xis) une trajectoire du processus $\xi_x$ cible ainsi que trois autres simulations de trajectoire du processus $\xi_x$ pour des valeurs de paramètres fixées, obtenues selon le procédé suivant :
...
g3 <- ggplot() +
  ...
  labs(caption = TeX("$\\xi_x^{(3)}$"), y = TeX("$\\xi$")) +
  theme(plot.margin = margin(0, 0, 0, 0, "pt"))

g4 <- ggplot() +
  geom_line(aes(x = x, y = xi)) +
  geom_line(aes(x = x, y = p3), color = "red") +
  labs(caption = TeX("$\\xi_x^{(4)}$")) +
...
Nous observons sur la Figure \@ref(fig:sim-xis) que plusieurs réalisations du processus $\xi$ conduisent à des trajectoires sensiblement différentes.
Ce résultat a un impact direct sur les observations $Y$ puisque comme nous pouvons le voir sur la Figure \@ref(fig:sim-ys), les observations $Y$ correspondant à des réalisations différentes ne collent pas parfaitement à la distribution cible.
Effectivement, la variabilité de simulation du paramètre $\xi$ entraine une variabilité sur les observations $Y$ qui se traduire par un décalage avec la distribution cible.
Les observations de $Y$ sont donc sensibles à la trajectoire du processus $\xi_x$ associé.

Nous ne pouvons donc pas nous contenter de tirer une réalisation de $\xi_k$ dans l'étape de simulation de notre algorithme, c'est pour cette raison que nous avons intégré à l'algorithme SAEM une étape MCMC ou SMC.
...
g2 <- ggplot() +
  geom_line(aes(x = x, y = Y)) +
  geom_line(aes(x = x, y = f(x, p1)), color = "green") +
  labs(caption = TeX("$Y^{(2)}$")) +
  theme(axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        plot.margin = margin(0, 0, 1, 0, "pt"))
...
g3 <- ggplot() +
  geom_line(aes(x = x, y = Y)) +
  geom_line(aes(x = x, y = f(x, p2)), color = "blue") +
  labs(caption = TeX("$Y^{(3)}$"), y = TeX("$Y$")) +
  theme(plot.margin = margin(0, 0, 0, 0, "pt"))

g4 <- ggplot() +
  geom_line(aes(x = x, y = Y)) +
  geom_line(aes(x = x, y = f(x, p3)), color = "red") +
  labs(caption = TeX("$Y^{(4)}$")) +
  theme(axis.title.y = element_blank(),
        plot.margin = margin(0, 0, 0, 0, "pt"))
...
Comme pour l'algorithme MCMC, le filtre particulaire à pour objectif de simuler une trajectoire de $\xi_x$ à partir des observations $Y_i$ accompagnée des paramètres $\gamma^2$, $\psi$ et$\omega^2$. Cet algorithme est éffectué via un algorithme "Sequential Monte Carlo" (SMC).

Les méthodes SMC sont basées sur la simulation afin de calculer des approximations de distributions postérieures. Elles permettent d'éviter les hypothèses de linéarité ou de normalité requises par d'autres méthodes. 

L'algorithme SMC consiste donc à utiliser un échantillonnage d'importance en incluant une étape d'anticipation. En effet, pour chaque instant $t$, le filtre particulaire calcule les poids $W_{t|t-1}$ pour les particules à partir du temps $t-1$. Ces poids sont calculés en utilisant une estimation de la probabilité des données actuelles étant donné chaque particule du point de temps précedent soit $P(Y_k(t)|\hat{\xi_kt))}$. Les particules avec un des poids de premier degré élevés correspondent à des valeurs de l'état latent à l'instant t-1 susceptibles de générer les données observées au temps t. En fonction des poids W.


\begin{algorithm}
\caption{Algorithme de filtre particulaire pour la simulation d'une trajectoire du processus $\xi_x$.}
\label{alg:SMC}
\begin{algorithmic}
\State $w \gets (w_1,....,w_n)$ \Comment{Initialisation du w adaptatif}
\State $w \gets 1/M$ \Comment{Initialisation du w adaptatif}
\For{$t \in \{1, ..., n\}$}
  \State$\hat{\xi_t} = f(\xi_t|\xi_{t-1})$ \Comment{Simulation de la valeur courante de $\xi$}
  \Comment{draw} \State$\hat{\xi_t} | \hat\xi_{t-1}$
  \State$W1 \gets P(Y(t)|\hat{\xi_t))}$ 
  \comment{Selectionner les indices pour $\xi_t$ et $W_t$ lorsque $W_t$ non nul}
  \State$W \gets \frac{W_t}{\sum{W_t}}$
  \State$idx \gets$ \Comment{Tirage probabiliste des particules en fonction de leur poids W}
  \Comment{Séléction des $\xi_k$ à l'indice $idx$ en fonction du tirage précedent}
\EndFor
\Comment{Tirage du dernier $\xi_n$ en fonction des poids $W_n$}
\end{algorithmic}
\end{algorithm}
...
À partir des résultats obtenus par les 1000 itérations, nous avons représenté sur la Figure \@ref(fig:params-ape) les boxplots des APE pour chacun des 7 paramètres.
...
Comme attendu, les $APE$ varient autour 0 pour tous nos paramètres hormis $\omega$.
En effet, $\omega$ semble être sous-estimé et avoir une variation plus grande.
Cependant, nous pouvons penser que cette variation est due au fait que la valeur de $\omega$ initiale de $0.01$ est faible, ce qui rend sa source de bruit difficile à distinguer des autres, plus importantes.
...
Au regard des Tables \@ref(tab:params-sum-mcmc) et \@ref(tab:params-sum-smc), il convient de relativiser l'erreur réalisée sur le paramètre $\omega$ : la valeur attendue est `r round(omega, 3)` et celle estimée en moyenne est égale à `r round(params.mean.smc[[1]], 3)`.
...
Finalement, les versions MCMC et SMC donnent des estimations extrêmement similaires.
Peut-être qu'il serait possible d'améliorer encore les résultats de la version SMC en jouant sur ses hyper-paramètres, mais cette démarche serait très coûteuse en temps de calcul, et potentiellement sensible aux choix des valeurs fixées des paramètres de la simulation.
