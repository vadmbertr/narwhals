...
params.smc.est <- readRDS("../../data/2_tusk/params.est.nls.a.rds")
```

```{r}
params.a.mean <- apply(params.smc.est, 2, mean)
params.a.ape <- t(apply(params.smc.est, 1, function (row) (params.true - row) / params.true) * 100)
params.a.mape <- apply(abs(params.a.ape), 2, mean)
params.a.se <- apply(params.smc.est, 1, function (row) (params.true - row)^2)
params.a.rmse <- sqrt(apply(params.a.se, 1, mean))
```

```{r}
df <- melt(params.a.ape, varnames = c("it", "param"))
ggplot(data = df) +
...
```{r}
params.true <- c(omega = omega, psi = psi, gamma = gamma, A = A, B = B, a = a, b = b)
params.a.b.est <- readRDS("../../data/2_tusk/params.est.mcmc.rds")
```

```{r}
params.a.b.mean <- apply(params.a.b.est, 2, mean)
params.a.b.ape <- t(apply(params.a.b.est, 1, function (row) (params.true - row) / params.true) * 100)
params.a.b.mape <- apply(abs(params.a.b.ape), 2, mean)
params.a.b.se <- apply(params.a.b.est, 1, function (row) (params.true - row)^2)
params.a.b.rmse <- sqrt(apply(params.a.b.se, 1, mean))
```

```{r}
df <- melt(params.a.b.ape, varnames = c("it", "param"))
ggplot(data = df) +
  geom_boxplot(aes(x = param, y = value))
```
...
```{r}
params.true <- c(omega = omega, psi = psi, gamma = gamma, A = A, B = B, a = a, b = b)
params.smc.est <- readRDS("../../data/2_tusk/params.est.smc.rds")
```

```{r}
params.smc.mean <- apply(params.smc.est, 2, mean)
params.smc.ape <- t(apply(params.smc.est, 1, function (row) (params.true - row) / params.true) * 100)
params.smc.mape <- apply(abs(params.smc.ape), 2, mean)
params.smc.se <- apply(params.smc.est, 1, function (row) (params.true - row)^2)
params.smc.rmse <- sqrt(apply(params.smc.se, 1, mean))
```

```{r}
df <- melt(params.smc.ape, varnames = c("it", "param"))
ggplot(data = df) +
  geom_boxplot(aes(x = param, y = value))
```
