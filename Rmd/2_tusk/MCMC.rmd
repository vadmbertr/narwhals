...
plot(delta[,indice],main="evolution du delta de la 456eme valeurs de xi") #comportement bizarre entre 50 et 100 
...
for (k in 2:M) {
  ...
  for (i in 1:n) {
    xi.p <- xi.c
    xi.p[[i]] <- rnorm(1, xi.p[[i]], delta[[k,i]])
    log.prob <- min(1, log.lik(xi.p,omega.arg=omega[k-1]) - log.lik(xi.c,omega.arg=omega[k-1]))
    if (log(runif(1)) < log.prob) {
      xi.c <- xi.p
      accepted.n[[i]] <- accepted.n[[i]] + 1
    }
