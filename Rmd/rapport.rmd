...
\qquad Dans le cadre du projet tutoré de notre 2\textsuperscript{ème} de Master Statistique et Science des Données nous nous sommes intéressés à deux questions concernant cet animal :
...
Merci également à Monsieur Frédéric Audra pour son aide quant à l'utilisation des plateformes de calcul du GRICAD.
...
où $\Delta$ est l’intervalle de temps entre deux observations.
...
\qquad L'algorithme EM est basé sur la log-vraisemblance du modèle $\log L(Y, \xi_x | \theta)$.
Etant donné que $p(Y_i | \xi_i) = \mathcal{N}(f_{\xi_i}(x_i, \varphi), \omega^2)$ et $p(\xi_i | \xi_{i-1}) = \mathcal{N}(\xi_{i-1} \psi, \gamma^2)$, celle-ci s'écrit de la manière suivante :
\begin{eqnarray*}
\log L(Y, \xi_x | \theta) &=& \sum_{i=1}^n \log p(Y_i | \xi_i) + \sum_{i=1}^n \log p(\xi_i | \xi_{i-1}) + \log p(\xi_1) \\
&=& -\sum_{i=1}^n \frac{(Y_i-f_{\xi_i}(x_i, \varphi))^2}{2\omega^2} - \frac{n}{2}\log(\omega^2) \\
...
$Q(\theta, \theta^{(k)}) = \mathbb{E}[\log L(Y, \xi_x | \theta) | Y, \theta^{(k)}]$
...
En remarquant que la densité complète du modèle appartient à la famille exponentielle, nous obtenons les définitions suivantes :

\begin{eqnarray*}
S_1(\xi_x)& =& \frac{1}{n} \sum_{i=1}^n (Y_i - f_{\xi_i}(x_i, \varphi))^2\\
S_2(\xi_x) &=& \sum_{i=1}^n \xi_{i-1} \xi_i\\
S_3(\xi_x) &=& \sum_{i=1}^n \xi_{i-1}^2\\
S_4(\xi_x) &=& \sum_{i=1}^n \xi_i^2
\end{eqnarray*}
...
Effectivement, après l'initialisation d'une trajectoire $\xi_x^{(0)} = (\xi^{(0)}_1,...,\xi^{(0)}_n)$, l'algorithme procède à $M$ itérations.
La trajectoire du processus simulée peut donc s'écrire $\xi_x^{(M)} = (\xi^{(M)}_1,...,\xi^{(M)}_n)$.

\qquad Plus précisément, pour chaque itération $k$, on calcule pour chaque position $x_i$, une valeur courante candidate $\xi_i^{(c)}$ avec une marche aléatoire : $\xi^{(c)}_i = \xi^{(k-1)}_i + N(0, \delta_i^2)$.
...
$$\log(\alpha) = min(\log(\frac{L(Y, \xi_x^{(c)})}{L(Y, \xi_x^{(k-1)})}), 1)$$
avec :
\begin{eqnarray*}
\log(\frac{L(Y, \xi_x^{(c)})}{L(Y, \xi_x^{(k-1)})}) &=& \log(L(Y, \xi_x^{(c)})) - \log(L(Y, \xi_x^{(k-1)})) \\
&=& -\frac{1}{2 \omega^2} \sum_{i=1}^{n} (Y_i - f_{\xi_i^{(c)}}(x_i, \varphi))^2 - \frac{1}{2 \frac{\gamma^2}{2}} \sum_{i=1}^{n} (\xi_i^{(c)} - \xi_i^{(k-1)} \psi)^2 \\
&& + \frac{1}{2 \omega^2} \sum_{i=1}^{n} (Y_i - f_{\xi_i^{(k-1)}}(x_i, \varphi))^2 + \frac{1}{2 \frac{\gamma^2}{2}}\sum_{i=1}^{n} (\xi_i^{(k-1)} - \xi_i^{(k-2)} \psi)^2\\
\end{eqnarray*}
...
\begin{eqnarray*}
\widehat{\varphi}^{(k)} &=& \arg\min_\varphi \sum_{i=1}^n \left(Y_i - f_{\xi_i^{(k)}}(x_i, \varphi)\right)^2
\\
...
\For{$q \in \{2, ..., Q\}$}
  \State \Comment{Etape E}
    \State $\xi^{(q)} \gets \xi$ avec $\theta^{(q-1)}$, par $M_q$ itérations MCMC (Algorithme \ref{alg:MCMC}), ou par SCM (Algorithme \ref{alg:SMC})

  \State \Comment{Etape SA}
    \State $S_1 \gets \frac{1}{n}\sum_{i=1}^n (Y_i - f_{\xi^{(q)}_i}(x_i, \varphi))^2$\Comment{Calcul des statistiques exhaustives}
  ...
  \State \Comment{Etape M}
    \State $\widehat{\varphi}^{(q)} \gets \arg\min_\varphi \sum_{i=1}^n \left(Y_i - f_{\xi_i^{(q)}}(x_i), \varphi)\right)^2$\Comment{Actualisation de $\theta^{(q)}$}
...
\qquad Au regard de la Table \@ref(tab:params-sum-mcmc), il convient de relativiser l'erreur réalisée sur le paramètre $\omega$ avec la méthode SAEM-MCMC : la valeur attendue est `r round(omega, 3)` et celle estimée en moyenne est égale à `r round(params.mean.mcmc[[1]], 3)`.
...
Nous pouvons noter que c'est bien le paramètre $b$ dont l'estimation est la plus éloignée de la valeur attendue, ce qui rejoint les observations faites à partir des Figures \@ref(fig:coef1-SAEM-mcmc) et \@ref(fig:coef2-SAEM-mcmc).
