...
Cette approche nécessite de fixer une mémoire maximale, et ainsi la valeur de $K$.
...
Le temps d'ajustement des modèles mixtes étant nettement plus important que ceux des modèles sans effets aléatoires, nous avons dans un premier temps exclu ces effets du modèle sans exposition pour estimer la mémoire optimale.
Nous avons choisi pour la recherche $K_{min} = 1\, K_{max} = 300\, M = 10$
Nous obtenons une mémoire optimale de `r maxlag.opt` secondes.
La figure \@ref(fig:plot-glm-bic) permet de voir que cet optimal semble bien correspondre à un minimum global.
...
De même que précédemment, nous obtenons une mémoire optimale de `r maxlag.glmer.opt` secondes et la figure \@ref(fig:plot-glmer-bic) nous indique qu'il s'agit bien d'un optimum global.
...
```{r plot-glm-time, fig.height = 3, fig.width = 6, fig.cap = "Temps d'ajustement (en secondes) des modèles en fonction de la mémoire maximum"}
