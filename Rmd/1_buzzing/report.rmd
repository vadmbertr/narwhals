...
\begin{align*}
    ...
    &\rightsquigarrow \mathcal{N}(0, \hat{\lambda(t)}^2 X(t)^T \hat{\Sigma} X(t)) \\
    \Leftrightarrow \frac{\sqrt{n}}{\hat{\lambda(t)} \sqrt{X(t)^T \hat{\Sigma} X(t)}} (\hat{\lambda(t)}) - \lambda(t)) &\rightsquigarrow \mathcal{N}(0, 1)
\end{align*}
Et on en déduit la bande de confiance au niveau $\alpha$ :
$$
IC_\alpha(\lambda(t)) = [\hat{\lambda(t)} \pm q_{1 - \alpha / 2}^\mathcal{N} \frac{\hat{\lambda(t)} \sqrt{X(t)^T \hat{\Sigma} X(t)}}{\sqrt{n}}]
$$
