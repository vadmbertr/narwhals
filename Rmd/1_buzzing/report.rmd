...
Ainsi, nous pouvons tenter de modéliser l'effet de l'exposition des perturbations humaines sur l'émission de buzz des narvals et donc indirectement sur leur alimentation.
...
Les processus de comptage sont classiquement représentés par un processus de Poisson : $N(t + \Delta) - N(t) \sim Pois$.
...
En se plaçant dans le cadre des modèles linéaires généralisés (GLM) il est donc possible d'exprimer la probabilité $\lambda(t)$ que $Y(t) = 1$ selon un ensemble de covariables $Z(t)$ (détaillées dans la suite de cette section) en utilisant le logarithme comme fonction de lien :
$$log(\lambda(t)) = \beta_0 + Z(t) \beta_Z$$
Ce cas particulier des GLM n'est autre que la régression logistique.
...
$$log(\lambda(t)) = \beta_0 + \textcolor{blue}{spline(D(t)) \beta_D} + ...$$
...
$$log(\lambda(t)) = \beta_0 + spline(D(t)) \beta_D + \textcolor{blue}{spline(E(t)) \beta_E} + ...$$
...
où $i$ dénote l'individu $i$ et $b_i \sim \mathcal{N}(O, \sigma_i^2)$ l'effet aléatoire sur cet l'individu, $\sigma_i^2$ étant la variance intra-individu de $i$.

### Caractère autorégressif du processus

\qquad De plus, l'émission d'un buzz à un instant $t$ est corrélé à l'émission ou non de buzz aux instants précédents ; cet effet mémoire doit donc être intégré au modèle pour rendre les $Y_{i}(t)$ indépendants.
...
\qquad Le nombre de composants autorégressifs pouvant être grand, nous avons utilisé une régression bi-exponentielle double permettant de lier le décalage mémoire et le coefficient associé :
...
Nous obtenons donc les estimations $\hat{\beta_D},\ \hat{A_1},\ \hat{lrc_1},\ \hat{A_2},\ \hat{lrc_2}$ qui sont ensuite injectées dans le modèle complet au moyen d'un terme d'\say{offset} :
$$offset_i(t) = spline(D_i(t)) \hat{\beta_D} + \sum_{k=1}^{K_{opt}} (\hat{A_1} e^{-e^{\hat{lrc_1}} k} + \hat{A_2} e^{-e^{\hat{lrc_2}} k}) Y_i(t-k)$$
...
Et les paramètres estimés $\hat{\beta_E}$ permettront d'interpréter l'effet de l'exposition par rapport à des conditions \say{normales}.
...
Nous avons donc utilisée l'approche Monte-Carlo suivante :

\begin{algorithmic}
\State $M \gets 1000$ \Comment{Nombre de répétitions de la procédure}
\State $\hat{\beta_E} \gets []$
\For{$i \in \{1, ..., M\}$}
    \Comment{Tirage aléatoire des coefficients de profondeur et autorégressifs}
    \State $\hat{\beta_D} \gets \mathcal{N}(\mathbb{E}(\beta_D), \mathbb{V}ar(\beta_D))$
    \State $\hat{A_1} \gets \mathcal{N}(\mathbb{E}(A_1), \mathbb{V}ar(A_1))$
    \State $\hat{lrc_1} \gets \mathcal{N}(\mathbb{E}(lrc_1), \mathbb{V}ar(lrc_1))$
    \State $\hat{A_2} \gets \mathcal{N}(\mathbb{E}(A_2), \mathbb{V}ar(A_2))$
    \State $\hat{lrc_2} \gets \mathcal{N}(\mathbb{E}(lrc_2), \mathbb{V}ar(lrc_2))$
    \State calcul de l'offset avec $\hat{\beta_D},\ \hat{A_1},\ \hat{lrc_1},\ \hat{A_2},\ \hat{lrc_2}$
    \State ajustement du modèle $\mathcal{M}$ en fixant l'offset
    \State $\hat{\beta_E}[i] \gets$ coefficients de profondeur ajustés de $\mathcal{M}$
\EndFor
\State $IC_\alpha \gets [Q_{\alpha / 2}^{\hat{\beta_E}}, Q_{1 - \alpha / 2}^{\hat{\beta_E}}]$ \Comment{Estimation via les quantiles empiriques}
\end{algorithmic}

### Estimation de bandes de prédiction via la méthode Delta {#delta}

\qquad Une fois les coefficients d'exposition $\hat{\beta_E}$ estimés nous pouvons prédire le taux d'émission de buzz sur l'ensemble d'un intervalle de niveau d'exposition aux perturbations.
Afin d'associer une bande de confiance à la prédiction moyenne nous avons appliqué la méthode Delta.

Tout d'abord, d'après le Théorème Central Limite on a : $\sqrt{n} (\hat{\beta} - \beta) \sim \mathcal{N}(0, \Sigma)$ avec $\hat{\beta}$ les coefficients estimés du modèle et $\Sigma$ leur variance.

Etant donné que nous utilisons une régression logistique, le taux d'émission est lié aux coefficients du modèle via la relation $\lambda(t) = exp(X(t) \beta) \vcentcolon = f(\beta)$.
Ce qui donne pour le gradient de la fonction $f$ : $\nabla f(\beta) = X f(\beta)$.

Ainsi, quand on applique la méthode Delta on obtient :
\begin{align*}
    \sqrt{n} (f(\hat{\beta}) - f(\beta)) &\xrightarrow[]{\mathcal{L}} \mathcal{N}(0, \nabla f(\beta)^T \Sigma \nabla f(\beta)) \\
    \Leftrightarrow \sqrt{n} (\hat{\lambda(t)}) - \lambda(t)) &\xrightarrow[]{\mathcal{L}} \mathcal{N}(0, \lambda(t)^2 X(t)^T \Sigma X(t)) \\
    &\rightsquigarrow \mathcal{N}(0, \hat{\lambda(t)}^2 X(t)^T \Sigma X(t)) \\
    \Leftrightarrow \frac{\sqrt{n}}{\hat{\lambda(t)}^2 X(t)^T \Sigma X(t)} (\hat{\lambda(t)}) - \lambda(t)) &\rightsquigarrow \mathcal{N}(0, 1)
\end{align*}
Et on en déduit la bande de confiance au niveau $\alpha$ :
$$
IC_\alpha(\lambda(t)) = [\hat{\lambda(t)} \pm q_{1 - \alpha / 2}^\mathcal{N} \frac{\hat{\lambda(t)}^2 X(t)^T \Sigma X(t)}{\sqrt{n}}]
$$
...
De même que précédemment, nous obtenons une mémoire optimale de `r maxlag.glmer.opt` secondes et la figure \@ref(fig:maxlag-bic) nous laisse penser qu'il s'agit d'un optimum global.
...
predFrame <- predFrame[predFrame$model == "With Depth", ]
predFramePop <- readRDS("../../data/1_buzzing/glmer_buzz_ARDepth_expo_pred/predFramePop.rds")
predFramePop <- predFramePop[predFramePop$model == "With Depth", ]
```
...
La figure \@ref(fig:mod-expo-rate-per) permet de voir que la bande de confiance tracée en rouge est fine et illustre que soumisent à des perturbations à une distance de 10 kilomètres, les narvals émettent deux fois moins de buzz que dans des conditions normales.

```{r}
ChangePop <- readRDS("../../data/1_buzzing/glmer_buzz_ARDepth_expo_pred/ChangePop.rds")
ChangePop <- ChangePop[ChangePop$model == "With Depth", ]
```
...
ggplot(data = ChangePop, aes(x = 1 / X, y = change)) +
  geom_line(size = .5) +
  geom_ribbon(aes(ymin = change - CI, ymax = change + CI),
              alpha = .5, linetype = 0, fill = "red") +
  xlim(0, 50) +
  xlab("Distance avec le bateau [km]") +
