...
Nous obtenons donc les estimations $\widehat{\beta_D},\ \widehat{A_1},\ \widehat{lrc_1},\ \widehat{A_2},\ \widehat{lrc_2}$ qui sont ensuite injectées dans le modèle complet au moyen d'un terme d'\say{offset} :
...
\begin{align*}
    ...
    &\rightsquigarrow \mathcal{N}(0, \widehat{\lambda(t)}^2 X(t)^T \widehat{\Sigma} X(t)) \\
    \Leftrightarrow \frac{\sqrt{n}}{\widehat{\lambda(t)} \sqrt{X(t)^T \widehat{\Sigma} X(t)}} (\widehat{\lambda(t)}) - \lambda(t)) &\rightsquigarrow \mathcal{N}(0, 1)
\end{align*}
...
```{r mod-expo-rate-per-merTools, fig.cap = "Pourcentage du taux normal d'émission de buzz selon la distance au bateau."}
# ggplot(data = ChangePop_merTools, aes(x = 1 / X, y = fit)) +
#   geom_line(size = .5) +
