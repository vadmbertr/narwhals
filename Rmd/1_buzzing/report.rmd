...
\While{$(to_k - from_k > 2)$} \Comment{On s'arrête quand on évalué l'ensemble du voisinage du minimum}
    ...
    \For{$i \in \{1, ..., M\}$}
        ...
        \State $\mathcal{M} \gets$ ajustement du modèle de l'Equation \@ref(eq:m-ard) avec $K[i]$ éléments mémoire
...
### Construction par approche Monte-Carlo {#mc}
...
\qquad Dans un premier temps, nous avons donc ajusté le modèle sans exposition décrit par l'Equation \@ref(eq:m0) afin d'obtenir la moyenne et la variance empiriques de ses coefficients.
...
\State $\widehat{\beta_{A_{1:2}}} \gets []$
\State $\widehat{\beta_{lrc_{1:2}}} \gets []$
\For{$k \in \{1, ..., K\}$}
    \State $\widehat{\alpha} \gets \mathcal{N}(\mu_{AR},\ \Sigma_{AR})$ \Comment{Tirage aléatoire des coefficients autorégressifs}
    \State $\mathcal{M}_{exp} \gets$ ajustement de la régression double bi-exponentielle utilisant $\widehat{\alpha}$
...
\State $\widehat{\beta_{E}} \gets []$
\For{$k \in \{1, ..., K\}$}
    \State $\widehat{\beta_{D}} \gets \mathcal{N}(\mu_D,\ \Sigma_D)$ \Comment{Tirage aléatoire des coefficients de profondeur}
    \State $\widehat{A_{1:2}},\ \widehat{lrc_{1:2}} \gets \mathcal{N}(\mu_{exp},\ \Sigma_{exp})$ \Comment{Tirage aléatoire des coefficients de la double bi-exponentielle}
...
Dans un premier temps, comme détaillé dans la section \@ref(mc), nous avons considéré que les coefficients autorégressifs et de profondeur suivaient chacun une loi normale centrée sur leur estimation moyenne et avec une variance égale au carré de leur erreur standard.
...
Pour plus de détails sur l'obtention des paramètres des lois normales, il est possible de revenir à la section \@ref(mc).
...
L'ensemble de cette procédure est décrite par l'Algorithme \@ref(alg:mc) de la section \@ref(mc).
