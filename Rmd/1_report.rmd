...
expo.coef <- readRDS("../data/glmer_buzz_ARDepth_expo_par/expo.coef.rds")
expo.coef <- as.data.frame(t(expo.coef))
expo.coef[, 1:ncol(expo.coef)] <- sapply(expo.coef[, 1:ncol(expo.coef)], as.numeric)
expo.coef <- expo.coef[ , colSums(is.na(expo.coef))==0]
expo.coef.confint <- t(apply(expo.coef, 1, percentile_conf_int))
expo.coef <- as.matrix(expo.coef)
```
