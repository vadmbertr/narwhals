...
## Comparaison de l'estimation du taux de buzzing avec et sans la profondeur
...
```{r}
dist <- seq(1, 70, .1) # distance to ship in km
depth <- -400 # fixed depth in m

predFrameMany <- NULL
for(k in unique(data$Ind)) {
  temp <- range(data$X[data$X > 0 & data$Ind == k])
  temp1 <- expand.grid(X = 1 / seq(1/temp[2], 1/temp[1], .1),
                       P = 0,
                       Depth = depth,
                       AR = 0,
                       Ind = k)
