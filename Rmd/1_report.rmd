...
```{r}
predFrame <- readRDS("../data/glmer_buzz_ARDepth_expo_plots/predFrame.rds")
predFramePop <- readRDS("../data/glmer_buzz_ARDepth_expo_plots/predFramePop.rds")
ChangePop <- readRDS("../data/glmer_buzz_ARDepth_expo_plots/ChangePop.rds")
```
...
ggplot(data = predFrame, aes(x = 1 / X, y = 60 * exp(predBuzz), color = Ind)) +
  ...
  # ylim(0, 0.3) +
  xlim(0, 50) +
  xlab("Distance avec le bâteau (km)") +
...
ggplot(data = ChangePop,
  ...
  geom_line() +
  ylim(0, 40) +
  ylab("Distance avec le bâteau (km)") +
  xlab("Pourcentage de taux normal d'émission de buzz") +
  # facet_grid(~ model)  +
