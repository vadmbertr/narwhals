...
```{r, fig.height = 3, fig.cap = "Variation des coefficients selon des lois normales univariées"}
g1 <- ggplot(ARcoefs.fit, aes(x = lag, y = truth)) +
  geom_line(data = ARcoefs.fit.mc, aes(x = lag, y = estimate, group = b), alpha = .1, col = "red") +
  geom_point(alpha = 0.5) +
  geom_line(aes(x = lag, y = estimate)) +
  geom_hline(yintercept = 0, alpha = .5, lty = 2) +
  labs(x = "Mémoire [secondes]", y = "Coefficient",
       caption = "Autorégressif") +
  theme(plot.margin = margin(0, 1, 0, 0, "pt"))

g2 <- ggplot(Depthcoefs.fit) +
...
```{r, fig.height = 3, fig.cap = "Variation des coefficients selon une loi normale multivariée"}
g1 <- ggplot(ARcoefs.fit, aes(x = lag, y = truth)) +
  geom_line(data = ARcoefs.fit.mc, aes(x = lag, y = estimate, group = b), alpha = .1, col = "red") +
  geom_point(alpha = 0.5) +
  geom_line(aes(x = lag, y = estimate)) +
...
#### Variance / covariance des coefficients autorégressifs et non des coefficients de la biexponentielle
